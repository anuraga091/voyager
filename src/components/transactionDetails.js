...
//   useEffect(() => {
//     // Fetch the transaction data from your backend or API
//     fetchTransactionData(transactionHash).then(data => setTransaction(data));
//   }, [transactionHash]);

//   const fetchTransactionData = async (hash) => {
//     // Replace with your API call logic
//     const response = await fetch(`http://localhost:3000/api/transactions/${hash}`);
    ...
    <div className="transaction-container">
        ...
        <div className="flex mt-8 w-96 justify-between	">
            <div>
                ...
                <p className='text-center mt-1 w-20 rounded border border-invokeBorder bg-invokeBackground text-invokeText text-base'>{`Invoke`.toUpperCase()}</p>
            </div> 
            <div className="flex flex-col justify-center  ">
                <span className="text-customGrayText text-lg">TIMESTAMP</span>
                <span className="text-white text-xl">{new Date(1657686576).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })} 
                    <span className="text-base ml-2">{new Date(1657686576).toLocaleTimeString()}</span>
                </span>
            </div>
        </div>
